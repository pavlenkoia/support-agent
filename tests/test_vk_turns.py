...
def test_human_override_suppresses_active_turn(tmp_path):
    session_factory = make_session(tmp_path)
    now = datetime.now(UTC)
    with session_factory() as session:
        item = conversation(session)
        turn = open_or_extend_turn(session, conversation_id=item.id, event_id=10, now=now, quiet_seconds=0)
        claim_due_turn(session, turn_id=turn.id, now=now, lease_seconds=60)
        assert suppress_active_turns(session, conversation_id=item.id, reason="human_override") == 1
        session.commit()
        stored = session.get(VkTurn, turn.id)
        assert stored is not None
        assert stored.status == "suppressed"
        assert stored.reason == "human_override"
