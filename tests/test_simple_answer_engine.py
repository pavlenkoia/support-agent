...
def test_natural_full_corpus_prompt_requires_natural_text_with_exact_evidence(tmp_path: Path) -> None:
    ...
    assert contract["rule"] == (
        "Верни один короткий ответ клиенту. Если последнее сообщение требует новой информации или действия, "
        "используй только факты из базы, которые прямо на него отвечают, и не добавляй неподходящие подтверждённые факты. "
        "Если последнее сообщение не требует новой информации или действия, ответь только коротким подтверждением его смысла; "
        "для такого подтверждения не используй базу знаний и историю диалога и не повторяй прежний ответ. "
