...
def test_natural_full_corpus_prompt_requires_natural_text_with_exact_evidence(tmp_path: Path) -> None:
    ...
    assert contract["rule"] == (
        "Верни один короткий ответ клиенту. Любой вопрос или запрос сведений, включая короткое уточнение по истории, "
        "требует содержательного ответа только по фактам из базы и не может быть социальной репликой. "
        "Только благодарность, прощание или подтверждение без вопроса и запроса сведений — социальная реплика: "
        "ответь на неё коротко и вежливо, без фактов из базы, истории, новых утверждений и повтора прежнего ответа. "
