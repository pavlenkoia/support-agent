...
def test_natural_full_corpus_prompt_requires_natural_text_with_exact_evidence(tmp_path: Path) -> None:
    ...
    assert contract["rule"] == (
        "Верни один короткий ответ клиенту. Если последнее сообщение является только благодарностью, прощанием или подтверждением "
        "без вопроса и просьбы сообщить сведения, ответь коротко и вежливо, например «Пожалуйста!». "
        "В такой реплике не используй базу знаний или историю и не повторяй прошлый ответ. "
...
def test_production_customer_prompt_prohibits_unsolicited_questions_and_evaluations() -> None:
    ...
    required_contract = (
        ...
        "Если последнее сообщение является только благодарностью, прощанием или подтверждением без вопроса и просьбы сообщить сведения, ответь коротко и вежливо, например «Пожалуйста!».",
        "В такой реплике не используй базу знаний или историю и не повторяй прошлый ответ.",
        "Во всех остальных случаях отвечай только по фактам из базы, которые прямо относятся к последнему сообщению.",
