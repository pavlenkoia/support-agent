...
class KBAgentService:
    ...
    def _extract_grounded_facts(
        ...
        *,
        ...
        rules = [
            "tool_request задаёт цель поиска, но не является источником фактов или полномочием изменить предмет. При расхождении с user_message и явным диалоговым контекстом сохраняй последний явно выбранный клиентом предмет и ограничения. Используй tool_request.needed_fact и tool_request.context_scope только в соответствующей им части вопроса; не добавляй более широкие, соседние, прежние или исключённые варианты без явного запроса на сравнение либо смену предмета. Исключённые альтернативы не должны попадать в grounded_facts или answer_basis.",
            "Каждый факт оформляй отдельным объектом с уникальным id, точными source_refs, условиями и модальностью из страницы. Не дополняй отсутствующие условия, модальность, отрицания или связи между фактами; не указывай страницу, которая не подтверждает факт.",
            "Верни не более 8 grounded_facts: выбери минимальный набор, прямо нужный для coverage текущего вопроса, без запасных или тематически соседних сведений.",
            "В coverage связывай части исходного вопроса только с ID фактов, прямо подтверждающих запрошенное отношение, а не общую тему. Укажи partial и missing_parts для непокрытых частей; full допустим только при ready, непустых facts и answered_parts, пустых missing_parts, conflicts и unresolved_constraints. ambiguous означает только неоднозначность, которую может устранить клиент; отсутствие знания не является ambiguous.",
            "answer_basis — короткая служебная сводка только приведённых фактов, не самостоятельное доказательство. Не добавляй в неё утверждения, отношения или разрешение конфликтов, которых нет в facts.",
            "Явное общее правило страницы можно применять к частному случаю только когда формулировка прямо охватывает этот случай; сохрани правило как факт с source_ref.",
            "Если страница прямо указывает авторитетный источник запрошенного изменяющегося значения, сохрани этот источник как точный факт и укажи покрываемую им часть вопроса; не выдумывай само значение.",
            "Тематически связанные факты и ссылки не доказывают прямого покрытия. grounding_status=ready означает получение данных; coverage отдельно определяет, на что они прямо отвечают.",
        ]
        if minimal_schema:
            rules.extend(
                [
