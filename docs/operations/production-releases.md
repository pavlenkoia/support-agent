...
The command obtains the full Git SHA, builds release-tagged images for `app`, `worker`, `vk-worker`, `viewer-web`, and `viewer-push-worker`, then force-recreates exactly those services. It never recreates `db` or its volume.
...
The command fails when any application-plane service is missing, not running, was not recreated for the operation, has a mismatched OCI revision label, or (for Python runtime services) has a mismatched complete `app/**/*.py` source manifest. It additionally requires backend health, Telegram/VK polling liveness, and a controlled `IncompleteRead` grounded-fallback probe in `app`, `worker`, and `vk-worker`.
