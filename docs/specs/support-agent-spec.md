...
20. Every accepted VK `message_new` must be durably stored as its own customer message before queueing. Queue coalescing may combine only the routing input; manual override or stale-generation suppression must never remove the original inbound messages from Viewer. Immediately before a VK reply is journaled and sent, the worker must re-check both transport-level override and whether the combined generation's newest external message ID still equals the durable latest-inbound marker for that conversation. A newer persisted inbound suppresses the obsolete generation.
