...
7. Only the common writer creates customer outcomes within structurally computed allowed_routes. Selector intent and dialogue do not prove business facts; the application does not force a tool by classifying a follow-up.
...
9. Acquisition success and semantic coverage are distinct. Clear unknown questions lead to natural `cannot_answer`; partial coverage or missing essential information requires a sourced profile fallback, and clarification is limited to genuine customer-resolvable ambiguity. `not_found` is not a technical retry; related facts or a candidate basis do not automatically promote it to `ready` or justify an answer.
...
15. Acquisition ready does not prove direct coverage. Only structurally consistent full Wiki coverage permits substantive finalization; non-answerable evidence supplies source-backed profile policy and original customer context but excludes neighboring facts and candidate basis from writer input. The original evidence remains in bounded audit. See [Final answer contract](final-answer-contract.md#evidence--allowed-outcome-current-stage-4-candidate).
...
5. On finalize, routing constructs an allowlisted evidence packet and invokes the existing common `respond` exactly once. Business facts remain Wiki-only; calendar observations stay separate. Navigation reasons, raw pages and audit metadata are not finalizer evidence. The stage-4 candidate may project the checked native history for terminal selection and audit, but it still must not hand raw extraction text to the writer.
