...
14. Broad but clearly in-domain openers with an explicit topic should prefer a safe overview answer over unnecessary clarification. An unresolved reference such as «эта услуга» remains a critical ambiguity: after KB lookup, a planner-approved clarification must be preserved rather than mechanically promoted to `answer_from_kb` from raw KB snippets.
