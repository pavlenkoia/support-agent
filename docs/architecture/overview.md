...
Each package has a monotonically increasing revision. A new source event or human override supersedes the active revision. Routing happens outside the polling flow; the final message persistence and transport send are serialized with enqueue/cancellation, so an obsolete revision cannot persist or deliver a customer reply. The VK worker journals the current Long Poll updates before recovery/replay. A provider `retry_pending` result is stored against its raw `transport_event` with a bounded next-attempt time and attempt count; replay atomically claims it as `processing` before generation, and expired claims are recovered. Before routing and again at the pre-send transaction, the worker compares the retry's external message ID with the latest inbound ID for that conversation: a newer client message or active human override suppresses the historical retry without delivery. Coalescing batch state itself remains deliberately process-local and is not replayed after a worker restart.
