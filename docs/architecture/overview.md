...
5. **Final-answer LLM failure does not erase grounded knowledge**. If KB facts were already gathered and the final answer-generation step fails, the runtime must return a short grounded fallback answer from the KB agent's extracted `grounded_facts` (or `answer_basis` when facts are unavailable), preserving their order. It must not synthesize from arbitrary full-page text that can introduce unrelated facts.
6. **Finalization receives a typed client-safe projection, not the KB-agent audit.** It keeps the complete mounted system prompt and client dialogue context, while excluding full selected KB pages, planner/navigation/review traces, route reasons, and raw tool output. The KB-agent's `answer_basis` is passed only as `candidate_synthesis`; grounded facts and the system prompt are authoritative when they differ. The finalizer must preserve semantic modality (`usually`, `may`, conditional language) and must not turn a general rule into a categorical answer for a specific case without explicit support.
