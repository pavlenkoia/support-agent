...
- **React + Tailwind viewer-web** published separately on port `3002` for read-only dialog browsing, with optional shared-key login, long-lived `HttpOnly` cookie sessions, installable online-first PWA shell, theme toggle, and mobile master-detail overlay navigation with slide-in / swipe-back detail transitions
