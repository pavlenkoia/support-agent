...
Each package has a monotonically increasing revision. A new source event or human override supersedes the active revision. Routing happens outside the polling flow; the final message persistence and transport send are serialized with enqueue/cancellation, so an obsolete revision cannot persist or deliver a customer reply. For Telegram, a provider `retry_pending` result is stored against its raw `transport_event` with a bounded next-attempt time and attempt count; the worker rebuilds and processes that due retry after a restart without creating a second user turn or delivery. Coalescing batch state itself remains deliberately process-local and is not replayed after a worker restart.
